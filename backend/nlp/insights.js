class InsightEngine {
  ...
  _kpiInsight(kpi) {
    ...
    return {
      text: `Terdapat ${total} sesi percakapan dengan rata-rata panjang ringkasan ${avg} karakter.`,
      type: 'metric',
    };
  }
  ...
  _topicInsight(topic) {
    ...
    return {
      text: `Topik "${top?.label || 'utama'}" mendominasi dengan ${pct}% dari seluruh pembahasan.`,
      type: 'topic',
    };
  }
  ...
  _commodityInsight(commodity) {
    ...
    const top = c[0];
    return {
      text: `Komoditas yang paling banyak dibahas adalah ${top.value} dengan ${top.count} sebutan.`,
      type: 'commodity',
    };
  }
  ...
  _intentInsight(intent) {
    ...
    const pct = top && total > 0 ? Math.round((top.count / total) * 100) : 0;
    return {
      text: `Intent "${top?._id || 'Informasi'}" mendominasi dengan ${pct}% dari total ${total} percakapan.`,
      type: 'intent',
    };
  }
  ...
  _trendInsight(trend) {
    ...
    if (g.direction === 'stabil')
      return { text: `Aktivitas percakapan cenderung stabil dalam periode yang dianalisis.`, type: 'trend' };
    const pct = Math.abs(g.percentChange);
    return {
      text: `Aktivitas percakapan ${dir} ${pct}%${g.direction === 'meningkat' ? ' dibanding periode sebelumnya' : ''}.`,
      type: 'trend',
    };
  }
  ...
  _coverageInsight(coverage) {
    ...
    return {
      text: `Skor coverage analitik mencapai ${overall}%, menunjukkan kualitas data yang ${overall > 70 ? 'baik' : overall > 40 ? 'cukup' : 'perlu ditingkatkan'}.`,
      type: 'metric',
    };
  }
  ...
  _sentimentInsight(intent) {
    ...
    if (posPct > negPct) {
      return {
        text: `Sentimen positif mendominasi dengan ${posPct}%, menunjukkan respons yang baik terhadap layanan chatbot.`,
        type: 'sentiment',
      };
    } else if (negPct > posPct) {
      return {
        text: `Sentimen negatif mencapai ${negPct}%, perlu perhatian lebih pada kualitas layanan.`,
        type: 'warning',
      };
    }
    return null;
  ...
  _relationshipInsight(_commodity) {
