class CoverageAnalyzer {
  ...
  _analyzeTopicCoverage(documents) {
    ...
    return {
      score: expected.length > 0 ? covered.length / expected.length : 0,
  ...
  _analyzeCommodityCoverage(nlpResults) {
    ...
    return {
      ...
      total: expected.length,
      covered: covered.length,
      commodities: allCommodities.slice(0, 20),
    };
  }
  ...
  _analyzeKnowledgeGraphCoverage(kg, nlpResults) {
    ...
    return {
      score: allEntities.size > 0 ? covered.length / allEntities.size : 0,
  ...
  _generateRecommendations(scores) {
    ...
    if (scores.commodityCoverage < 0.5)
      recs.push('Cakupan komoditas masih rendah. Fokus pada komoditas utama seperti beras, jagung, dan cabai.');
    if (scores.locationCoverage < 0.5) recs.push('Cakupan lokasi terbatas. Perlu data dari lebih banyak provinsi.');
    if (scores.intentCoverage < 0.5)
      recs.push('Distribusi intent belum merata. Dorong percakapan dengan berbagai tujuan.');
    if (scores.entityCoverage < 0.7)
      recs.push('Beberapa tipe entitas belum terdeteksi. Perkaya pola ekstraksi entitas.');
