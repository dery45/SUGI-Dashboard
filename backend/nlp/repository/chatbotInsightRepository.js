...
class ChatbotInsightRepository {
  async getKPIs(filters) {
    ...
    const [
      ...
      SessionSummary.aggregate([{ $match: match }, { $group: { _id: null, total: { $sum: '$char_count' } } }]).then(
        (r) => r[0]?.total || 0
      ),
      SessionSummary.aggregate([{ $match: match }, { $group: { _id: null, avg: { $avg: '$char_count' } } }]).then((r) =>
        Math.round(r[0]?.avg || 0)
      ),
