...
class EntityExtractor {
  extract(text) {
    ...
    const measurementMatch = lower.match(
      /(\d+[.,]?\d*)\s*(kg|liter|ton|kwintal|hektar|meter|cm|mm|ha|ml|gr|gram|kilogram|kilo|kw|ton)/gi
    );
    if (measurementMatch) {
    ...
    const dateMatch = lower.match(
      new RegExp('\\b(\\d{1,2}\\s+[a-z]+(?:\\s+\\d{4})?|\\d{4}-\\d{2}-\\d{2}|\\d{2}/\\d{2}/\\d{4})\\b', 'gi')
    );
    if (dateMatch) {
      dateMatch.forEach((d) => {
    ...
    return Array.from(entitiesMap.values()).sort((a, b) => entityOrder.indexOf(a.type) - entityOrder.indexOf(b.type));
  }
