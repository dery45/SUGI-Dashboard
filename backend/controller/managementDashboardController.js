...
const calculateOrganizationOverview = async ({ farm_id, start_date, end_date } = {}) => {
  try {
    const saleMatch = buildMatch({ farm_id, start_date, end_date });
    const farmFilter = farm_id ? { _id: farm_id, status: 'Active' } : { status: 'Active' };
    const cycleFilter = farm_id
      ? { farm_id, status: { $in: ['Planned', 'Land_Preparation', 'Planted', 'Maintenance', 'Harvesting'] } }
      : { status: { $in: ['Planned', 'Land_Preparation', 'Planted', 'Maintenance', 'Harvesting'] } };

    const [activeFarmsCount, activeCycles, unassignedUMs, salesAgg, expenseAgg] = await Promise.all([
      ...
      Expense.aggregate([{ $match: saleMatch }, { $group: { _id: null, totalCost: { $sum: '$amount_idr' } } }]),
    ]);
    ...
    const farmAreaAgg = await FarmMaster.aggregate([
      { $match: farmFilter },
...
const getYieldTrend = async ({ farm_id, year, start_date, end_date } = {}) => {
  try {
    ...
    const trend = await Sale.aggregate([
      ...
      {
        $group: {
          _id: { month: { $month: '$sale_date' }, buyer_type: '$buyer_type' },
...
const generateAlerts = async ({ farm_id } = {}) => {
  const alerts = [];
  const now = new Date();
...
const getKpi = async (req, res) => {
  try {
    ...
    const data = await calculateOrganizationOverview({ farm_id, start_date, end_date });
    const alerts = await generateAlerts({ farm_id });
...
const getYieldTrendHandler = async (req, res) => {
  try {
    ...
    const trend = await getYieldTrend({ farm_id, year, start_date, end_date });
...
const getUMPerformanceHandler = async (req, res) => {
  try {
    const data = await getUMPerformance();
...
module.exports = { getKpi, getYieldTrendHandler, getUMPerformanceHandler };
