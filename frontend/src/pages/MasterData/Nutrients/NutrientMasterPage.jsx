import InputMasterPage from '../Fertilizers/FertilizerMasterPage';
import React from 'react';
// Re-export thin wrapper to avoid duplication; Nutrient uses same generic component with different endpoint
  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in pb-16">
      <div className="bg-gradient-to-br from-surface/60 via-surface/30 to-transparent backdrop-blur-xl p-8 rounded-[2.5rem] border border-border/30 shadow-lg">
        <div className="flex items-center justify-between"><div className="flex items-center gap-3"><div className="w-2 h-8 bg-gradient-to-b from-emerald-400 to-emerald-600 rounded-full" /><div><h1 className="text-2xl font-black text-foreground tracking-tight">Master Data Nutrisi</h1><p className="text-muted text-xs font-bold uppercase tracking-[0.25em] opacity-60 mt-0.5">Kelola nutrisi tanaman</p></div></div><div className="flex gap-3"><button onClick={fetchData} className="p-2.5 rounded-xl border border-border/40 hover:border-primary hover:text-primary transition-all"><RefreshCw className="w-4 h-4" /></button><button onClick={() => { setEditItem(null); setForm({ name: '', unit: '', description: '', status: 'Active' }); setErrors({}); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl hover:bg-primary/90 transition-all font-bold text-xs uppercase tracking-wider"><Plus className="w-4 h-4" /> Tambah Nutrisi</button></div></div>
      </div>
      <Card title="Daftar Nutrisi"><DataTable columns={columns} data={data} onView={setViewModal} onEdit={handleEdit} onDelete={handleDelete} itemsPerPage={10} /></Card>
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          ...
          <div className="relative w-full max-w-lg bg-surface border border-border/40 rounded-[2rem] shadow-2xl p-8" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-black text-foreground mb-6">{editItem ? 'Edit Nutrisi' : 'Tambah Nutrisi Baru'}</h2>
            <div className="grid grid-cols-1 gap-4">
              <Input label="Nama Nutrisi" name="name" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} error={errors.name} />
      ...
      {viewModal && <ViewDetailModal isOpen={!!viewModal} onClose={() => setViewModal(null)} record={{ ...viewModal, unit: viewModal.unit ? `${viewModal.unit.name} (${viewModal.unit.symbol})` : '-' }} columns={columns} title="Detail Nutrisi" />}
    </div>
  );
};
export default NutrientMasterPage;
