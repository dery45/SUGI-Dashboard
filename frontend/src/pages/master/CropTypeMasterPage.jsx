...
const CropTypeMasterPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(`${BASE_URL}/master-data/crop-types`, { headers });
  ...
  const handleSave = async () => {
    if (!validate()) return;
    const url = editItem ? `${BASE_URL}/master-data/crop-types/${editItem._id}` : `${BASE_URL}/master-data/crop-types`;
      ...
      <div className="bg-gradient-to-br from-surface/60 via-surface/30 to-transparent backdrop-blur-xl p-8 rounded-[2.5rem] border border-border/30 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-foreground tracking-tight">Master Data Jenis Tanaman</h1>
              <p className="text-muted text-xs font-bold uppercase tracking-[0.25em] opacity-60 mt-0.5">Kelola jenis tanaman</p>
            </div>
          </div>
...
export default CropTypeMasterPage;
